...
bool intern_wotc_bs_62_move1_condition(const struct database_move* move) {
	printf("Checked %s\n", move->name[ENGLISH]);
	return true;
};

// Function to run when move is used
void intern_wotc_bs_62_move1_execute(const struct database_move* move) {
    printf("Used %s\n", move->name[ENGLISH]);
};

// Struct of move
const struct database_move intern_wotc_bs_62_move1 = {
	...
	.condition = &intern_wotc_bs_62_move1_condition,
	.execute = &intern_wotc_bs_62_move1_execute,
};

// Array of all move structs
const struct database_move* intern_wotc_bs_62_moves[] = {
	&intern_wotc_bs_62_move1,
};

// Function to check if ability can be used
bool intern_wotc_bs_62_ability_condition(const struct database_ability* ability) {
	printf("Checked %s\n", ability->name[ENGLISH]);
	return true;
};

// Function to run for ability
void intern_wotc_bs_62_ability_execute(const struct database_ability* ability) {
    printf("Used %s\n", ability->name[ENGLISH]);
};

// Struct of ability
const struct database_ability intern_wotc_bs_62_ability = {
	.name = (const char* [LANGUAGE]) {
		[ENGLISH] = "Damage Swap",
        [JAPANESE] = "",
        [GERMAN] = "",
	...
	.condition = &intern_wotc_bs_62_ability_condition,
	.execute = &intern_wotc_bs_62_ability_execute,
};

// Struct to contain everything about the card
const struct database_card intern_wotc_bs_62= {
	...
	.moves = intern_wotc_bs_62_moves,
	.move_count = sizeof(intern_wotc_bs_62_moves)/sizeof(intern_wotc_bs_62_moves[0]),
	.ability = &intern_wotc_bs_62_ability,
    .prize = 1,
    // Even Japanese cards use english letters for names
    .illustrator = "Ken Sugimori",
	.images = (const struct embedded*[LANGUAGE]) {
		[ENGLISH] = &(const struct embedded) {
			.data = intern_wotc_bs_62_en_png,
			.len = intern_wotc_bs_62_en_png_len,
		},
	},
